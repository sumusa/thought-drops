-- Migration: Update reaction system to allow only one reaction per user per echo
-- Users can change their reaction type, but can't have multiple reactions on the same echo

-- Update the unique constraint to be per user per echo (not per reaction type)
ALTER TABLE echo_reactions DROP CONSTRAINT IF EXISTS echo_reactions_user_id_echo_id_reaction_type_key;
ALTER TABLE echo_reactions ADD CONSTRAINT echo_reactions_user_id_echo_id_key UNIQUE(user_id, echo_id);

-- Updated function to handle one reaction per user per echo
